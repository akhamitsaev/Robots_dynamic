...
1. Multistep rollout накапливает ошибку, так как каждый следующий шаг зависит от предыдущего прогноза.
2. KMeans-кластеры являются data-driven состояниями и требуют физической интерпретации.
3. Для более устойчивой статистики synthetic study желательно увеличить `repeats`.
