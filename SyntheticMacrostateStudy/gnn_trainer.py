...
class GNNTrainer:
    """Port of StepPrediction/gnn_trainer.py from the GitHub project.

    This class intentionally preserves the one-step training/evaluation behavior:
    - GraphDataset precomputes all graphs for file splits;
    - StandardScaler is fitted on train node/edge/target tensors;
    - batches are normalized inside collate_fn;
    - metrics and saved predictions are denormalized through target_scaler.
    """
    ...
    def train_and_evaluate(self, experiments_dict, kinematics_dict, train_files, val_files, test_files):
        ...
        train_dataset = GraphDataset(experiments_dict, kinematics_dict, train_files, self.config.k_neighbors, self.config)
        val_dataset = GraphDataset(experiments_dict, kinematics_dict, val_files, self.config.k_neighbors, self.config)
        test_dataset = GraphDataset(experiments_dict, kinematics_dict, test_files, self.config.k_neighbors, self.config)

        print(f"📊 Размеры данных: Train={len(train_dataset)}, Val={len(val_dataset)}, Test={len(test_dataset)}")
        self._fit_scalers(train_dataset)

        train_loader = DataLoader(train_dataset, batch_size=self.config.batch_size, shuffle=True, num_workers=0, collate_fn=self.graph_collate_fn)
        val_loader = DataLoader(val_dataset, batch_size=self.config.batch_size, shuffle=False, num_workers=0, collate_fn=self.graph_collate_fn)
        test_loader = DataLoader(test_dataset, batch_size=self.config.batch_size, shuffle=False, num_workers=0, collate_fn=self.graph_collate_fn)
        ...
        # Original GitHub behavior: after training/evaluation, run one-step
        # inference for every provided experiment and save results/gnn_predictions.parquet.
        # It is intentionally configurable here because this stage may take a long
        # time and is mainly needed for one-step video / saved prediction tables.
        if getattr(self.config, "save_one_step_predictions_after_training", True):
            all_predictions = self._predict_all(experiments_dict, kinematics_dict)
            self._save_results(all_predictions)
        else:
            print("⏭️ Пропущено сохранение full one-step predictions "
    ...
    def _train(self, train_loader, val_loader):
        ...
        epoch_bar = tqdm(range(self.config.num_epochs), desc="🎯 Обучение GNN", disable=not self.config.show_progress)
    ...
    def _train_epoch(self, train_loader, epoch):
        ...
        batch_bar = tqdm(train_loader, desc=f'Epoch {epoch+1}/{self.config.num_epochs}', leave=False, disable=not self.config.show_progress)
    ...
    def _evaluate(self, test_loader, phase="Тестирование"):
        ...
        all_predictions_denorm = torch.tensor(self.target_scaler.inverse_transform(all_predictions.numpy()), dtype=torch.float32)
        all_targets_denorm = torch.tensor(self.target_scaler.inverse_transform(all_targets.numpy()), dtype=torch.float32)
    ...
    def _predict_all(self, experiments_dict, kinematics_dict):
        ...
        This keeps the original GitHub behavior of saving one-step predictions
        for every provided file, but adds a progress bar because this stage can
        be slower than expected on large datasets.
        """
        ...
        for exp_data in experiments_dict.values():
            ...
            n_graphs = max(0, n_timesteps - 3)  # t = 2 ... n_timesteps - 2
            total_graphs += n_graphs
            total_rows += n_graphs * n_robots
        print(f"   Графов для inference: {total_graphs}")
        print(f"   Строк predictions:    ~{total_rows}")

        progress = tqdm(
            total=total_graphs,
            desc="🔮 One-step inference",
            disable=not getattr(self.config, "show_progress", True),
        )

        with torch.no_grad():
            for file_idx, (file_name, exp_data) in enumerate(experiments_dict.items(), start=1):
                ...
                for t in range(2, n_timesteps - 1):
                    ...
                    progress.update(1)
                    if progress.n % 200 == 0 or progress.n == total_graphs:
                        progress.set_postfix({
                            "file": f"{file_idx}/{len(experiments_dict)}",
                            "t": t,
                        })
